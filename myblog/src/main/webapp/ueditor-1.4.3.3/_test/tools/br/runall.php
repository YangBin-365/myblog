...
function run( $b , $debug = false )
{
    ...
    if(array_key_exists(  "mefilter" , $_GET  )){
        $filterR = array_key_exists( $b , $_GET )?$_GET[$b]:$_GET['mefilter'];
        if(strstr($b,'main')||strstr($b,'supp')){
            $url .= "^&filterRun={$filterR}^&mefilter={$_GET['mefilter']}";
        }else {
            $url .= "^&filterRun={$_GET['mefilter']}^&mefilter={$_GET['mefilter']}";
...
$reportfile = "report_{$_GET['mefilter']}";
