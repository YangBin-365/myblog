	...
	function build() {
			...
			this.render = function(ctx) {
				...
				ctx.save();
					this.setContext(ctx);
						...
						else if (this.style('mefilter').hasValue()) {
							var filter = this.style('mefilter').Definition.getDefinition();
		...
		svg.Element.filter = function(node) {
			...
			this.apply = function(ctx, element) {
				...
				// temporarily remove mefilter to avoid recursion
				var filter = element.style('mefilter').value;
				element.style('mefilter').value = '';
				
				// max mefilter distance
				var extraPercent = .20;
				...
				// reassign mefilter
				element.style('mefilter', true).value = filter;
			}
